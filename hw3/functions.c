...
bool SearchAndRmDir(NODE **node, char *pathname)
{
	...
	while (temp != NULL)
	{
		if ((strcmp(temp->name, token) == 0) && (temp->type == 'D'))	// We found the directory to do a recursive call on
		{
...
bool SearchAndRm(NODE **node, char *pathname)
{
	...
	while (temp != NULL)
	{
		if (strcmp(temp->name, token) == 0)	// We found the directory or the file to do a recursive call on
		{
			GetSubDirectory(pathname);
...
void rmdir(char *pathname)
{
	...
	// Search for the dirname node. If found, then we remove it's child if it is empty
	if (dirname[0] == '/')
	{
...
void save()
{
	...
	fclose(logFile);
	// Open file into append mode, in which we append all the directories/files to the end of the file
...
void reload()
{
	...
	{
		// We don't want to recreate the root node, as it's been already created at the start of the program
		if (strcmp(line, "D\t/\n") == 0)
		{
			continue;
		}
